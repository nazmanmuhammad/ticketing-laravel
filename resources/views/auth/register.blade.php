...
<div class="flex min-h-screen" x-data="{ show: false, showPw: false, showPwC: false, submitting: false }" x-init="setTimeout(() => show = true, 100)">
    ...
    <div class="flex-1 flex items-center justify-center p-6 sm:p-10">
        <div x-show="show" x-transition:enter="transition ease-out duration-600" x-transition:enter-start="opacity-0 translate-y-6" x-transition:enter-end="opacity-100 translate-y-0" class="w-full max-w-md">
            ...
            <div class="mb-8">
                ...
                <p class="text-secondary text-sm mt-2">Fill in the details below to get started</p>
            </div>
            ...
            <div class="mb-6 p-4 rounded-xl bg-red-50 border border-red-200 text-red-700 text-sm" x-data="{ s: true }" x-show="s" x-transition>
                <div class="flex items-start gap-2">
                    <svg class="size-5 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.34 16.5c-.77.833.192 2.5 1.732 2.5z"/></svg>
            ...
            <form method="POST" action="{{ route('register') }}" class="space-y-4" @submit="submitting = true">
                @csrf
                <div>
                    <label for="name" class="block text-sm font-semibold text-foreground mb-2">Full name<span class="text-error">*</span></label>
                ...
                <div>
                    <label for="password_confirmation" class="block text-sm font-semibold text-foreground mb-2">Confirm password<span class="text-error">*</span></label>
                    <div class="relative">
                        <input id="password_confirmation" :type="showPwC ? 'text' : 'password'" name="password_confirmation" required
                               class="w-full h-12 px-4 pr-12 rounded-xl border border-border bg-white text-sm font-medium focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all duration-200"
                               placeholder="Confirm your password">
                        <button type="button" @click="showPwC = !showPwC" class="absolute right-4 top-1/2 -translate-y-1/2 text-secondary hover:text-foreground transition-colors cursor-pointer">
                            <svg x-show="!showPwC" class="size-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                            <svg x-show="showPwC" class="size-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"/></svg>
                        </button>
                    </div>
                </div>

                <button type="submit" :disabled="submitting" class="w-full h-12 bg-primary hover:bg-primary-hover text-white rounded-full font-semibold shadow-lg shadow-primary/20 hover:shadow-primary/30 transition-all duration-200 hover:scale-[1.01] active:scale-[0.98] cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:scale-100 flex items-center justify-center gap-2 mt-2">
