<!-- Top Navbar -->
<div class="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-border shrink-0">
    <div class="flex items-center justify-between w-full h-[64px] px-5 md:px-8">
        <div class="flex items-center gap-3">
            ...
            <button @click="$dispatch('toggle-sidebar')" class="hidden lg:flex size-10 items-center justify-center rounded-xl ring-1 ring-border hover:ring-primary hover:bg-muted transition-all duration-200 cursor-pointer" title="Toggle Sidebar">
                <i data-lucide="panel-left" class="size-5 text-secondary"></i>
            </button>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open" class="flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-muted transition-all duration-200 cursor-pointer">
                    ...
                    <img src="{{ Storage::url(auth()->user()->profile_photo) }}" alt="{{ auth()->user()->name }}" class="size-8 rounded-full object-cover">
                    @else
    ...
    <!-- Page Title & Breadcrumb Sub-bar -->
    <div class="flex items-center justify-between px-5 md:px-8 py-2.5 border-t border-border bg-white">
        <h2 class="font-bold text-sm text-foreground">@yield('page-title', 'Dashboard')</h2>
        <nav class="hidden sm:flex items-center gap-1 text-sm text-secondary">
