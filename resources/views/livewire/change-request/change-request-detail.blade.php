@section('title', $changeRequest->request_number . ' - ' . $changeRequest->title)
...
<span class="text-secondary">&middot;</span>
<span class="font-semibold text-foreground">{{ $changeRequest->request_number }}</span>
@endsection
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-border p-6">
                <div class="flex items-start justify-between mb-4">
                    <div>
                        <h2 class="text-lg font-bold text-foreground">{{ $changeRequest->title }}</h2>
                        <p class="text-xs text-secondary mt-1">Created by {{ $changeRequest->requester?->name }} &middot; {{ $changeRequest->created_at->diffForHumans() }}</p>
                    </div>
                    @if($changeRequest->requester_id === auth()->id() && in_array($changeRequest->status, ['draft', 'submitted', 'under_review', 'info_requested']))
                    <a href="{{ route('change-requests.edit', $changeRequest) }}" wire:navigate
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    ...
                    <div><span class="text-secondary block mb-1">Scheduled</span><span class="font-medium text-foreground">{{ $changeRequest->scheduled_at->format('M d, Y H:i') }}</span></div>
                    @endif
                </div>
                <div class="mt-4 pt-4 border-t border-border">
                    <span class="text-secondary text-sm block mb-1">Description</span>
                    <div class="prose prose-sm max-w-none text-foreground/80">{!! nl2br(e($changeRequest->description)) !!}</div>
                </div>
            ...
            <div class="bg-white rounded-2xl border border-border p-6">
                ...
                <div class="space-y-3 mb-4">
                    @foreach($changeRequest->approvals->sortBy('level') as $approval)
                    <div class="flex items-center justify-between p-3 rounded-xl border {{ $approval->status === 'approved' ? 'border-green-200 bg-green-50' : ($approval->status === 'rejected' ? 'border-red-200 bg-red-50' : ($approval->status === 'info_requested' ? 'border-yellow-200 bg-yellow-50' : 'border-border bg-muted/30')) }}">
                        ...
                        <div class="text-right">
                            @php
                                $badgeColors = ['pending' => 'bg-gray-100 text-gray-600', 'approved' => 'bg-green-100 text-green-700', 'rejected' => 'bg-red-100 text-red-700', 'info_requested' => 'bg-yellow-100 text-yellow-700', 'rescheduled' => 'bg-yellow-100 text-yellow-700'];
                            @endphp
                ...
                <div class="border-t border-border pt-4 space-y-3">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button @click="$dispatch('confirm-dialog', { title: 'Approve Change Request', message: 'Are you sure you want to approve this change request?', confirmText: 'Approve', onConfirm: () => $wire.approve() })" class="h-10 px-5 bg-green-600 hover:bg-green-700 text-white rounded-xl text-sm font-semibold transition-all duration-200 active:scale-95 cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-2" wire:loading.attr="disabled" wire:target="approve">
                        ...
                        <button @click="$dispatch('confirm-dialog', { title: 'Reject Change Request', message: 'Are you sure you want to reject this change request? Notes are required.', confirmText: 'Reject', onConfirm: () => $wire.reject() })" class="h-10 px-5 bg-red-600 hover:bg-red-700 text-white rounded-xl text-sm font-semibold transition-all duration-200 active:scale-95 cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-2" wire:loading.attr="disabled" wire:target="reject">
            ...
            <div class="bg-white rounded-2xl border border-border overflow-hidden">
                <div class="flex border-b border-border">
                    <button @click="activeTab = 'comments'" :class="activeTab === 'comments' ? 'border-primary text-primary' : 'border-transparent text-secondary hover:text-foreground'"
                        ...
                        Comments ({{ $changeRequest->comments->count() }})
                    </button>
        ...
        <div class="space-y-4">
            <div class="bg-white rounded-2xl border border-border p-5">
                ...
                @php $sColors = ['draft'=>'bg-gray-100 text-gray-600','submitted'=>'bg-blue-50 text-blue-700','under_review'=>'bg-yellow-50 text-yellow-700','pending_approval'=>'bg-amber-50 text-amber-700','info_requested'=>'bg-orange-50 text-orange-700','rejected'=>'bg-red-50 text-red-700','approved'=>'bg-green-50 text-green-700','scheduled'=>'bg-indigo-50 text-indigo-700','implemented'=>'bg-purple-50 text-purple-700','closed'=>'bg-gray-100 text-gray-600','failed'=>'bg-red-50 text-red-700']; @endphp
            ...
            <div class="bg-white rounded-2xl border border-border p-5 space-y-3">
                ...
                <div class="space-y-2 text-sm">
                    ...
                    <div class="flex justify-between"><span class="text-secondary">System</span><span class="font-medium">{{ $changeRequest->system?->name }}</span></div>
                    ...
                    <div class="flex justify-between"><span class="text-secondary">Assigned To</span><span class="font-medium">{{ $changeRequest->assignee->name }}</span></div>
                    @endif
                    @if($changeRequest->team)
                    <div class="flex justify-between"><span class="text-secondary">Team</span><span class="font-medium">{{ $changeRequest->team->name }}</span></div>
                    @endif
                    @if($changeRequest->scheduled_at)
                    <div class="flex justify-between"><span class="text-secondary">Scheduled</span><span class="font-medium">{{ $changeRequest->scheduled_at->format('M d, Y H:i') }}</span></div>
                    @endif
                    @if($changeRequest->implemented_at)
                    <div class="flex justify-between"><span class="text-secondary">Implemented</span><span class="font-medium">{{ $changeRequest->implemented_at->format('M d, Y H:i') }}</span></div>
                    @endif
                    <div class="flex justify-between"><span class="text-secondary">Created</span><span class="font-medium">{{ $changeRequest->created_at->format('M d, Y H:i') }}</span></div>
                </div>
            </div>
        </div>
    </div>
